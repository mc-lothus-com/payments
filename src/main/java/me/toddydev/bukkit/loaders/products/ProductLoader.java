...
public class ProductLoader {
    ...
    public static void load(JavaPlugin plugin) {
        ...
        for (File f : files) {
            ...
            List<String> hologram = config.getStringList("npc.hologram");

            hologram.replaceAll(l -> l.replace("&", "§"));

            ProductNPC npc = new ProductNPC();
            npc.setHologram(hologram);
            npc.setLocation(createLocation(config.getString("npc.location")));
            npc.setSkin(new Skin(config.getString("npc.skin.value"), config.getString("npc.skin.signature")));
            ...
            product.setCash(config.getInt("cash"));
