...
public class ImageCreator {
    ...
    public static void generateMap(String data, Player player) {
        try {
            ...
            List<String> lore = BukkitMain.getMessagesConfig().getStringList("item-qrcode-description");
            lore.replaceAll(line -> line.replace("&", "§"));
            ItemStack item = new ItemBuilder(Material.MAP, mapView.getId())
                    .name(BukkitMain.getMessagesConfig().getString("item-qrcode-name").replace("&", "§"))
                    .lore(
                            lore
                    ).build();
            ...
            component.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText(
                    BukkitMain.getMessagesConfig().getString("success-payment-link-hover")
                            .replace("&", "§")
                            .replace("{nl}", "\n"))));
